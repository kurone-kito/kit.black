...
- no dependency install step →
  `pnpm install --prefer-frozen-lockfile = true`
- no relevant auto-fix command →
  `pnpm run lint:fix && pnpm run lint = true`

Keep `pnpm install --prefer-frozen-lockfile` safe to rerun across retries, takeovers,
and recreated worktrees.

## Marker prefix notes

`kit-black` appears in two hidden issue-body markers:

- roadmap identity marker:
  `<!-- kit-black-roadmap-id: {unique-id} -->`
- blocked-by marker:
