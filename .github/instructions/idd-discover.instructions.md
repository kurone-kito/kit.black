...
3. Apply A3's readiness bullets to the target — no configured
   ...
   `kit-black-blocked-by` marker, both resolved the
...
NOT contain a `kit-black-roadmap-id` marker (not itself
a roadmap) or a `kit-black-blocked-by` marker, AND
...
- **Roadmap identity** (`kit-black-roadmap-id`): placed in
...
- **Sequential dependency** (`kit-black-blocked-by`): placed
