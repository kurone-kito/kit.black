...
2. If the target issue carries the configured roadmap label or an
   `kit-black-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `kit-black-blocked-by` markers) and its
...
NOT contain an `kit-black-roadmap-id` marker (not itself
a roadmap) or an `kit-black-blocked-by` marker, AND
...
`kit-black-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`kit-black-roadmap-id` marker. Retro-label a legacy
...
`<!-- kit-black-roadmap-id: ... -->` marker are
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- kit-black-roadmap-id: {roadmap-id} -->`; treat as
...
Read the authored `<!-- kit-black-effort: S|M|L -->` footer
...
- **Roadmap identity** (`kit-black-roadmap-id`): in the
...
- **Sequential dependency** (`kit-black-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `kit-black-blocked-by` to group sub-tasks under
